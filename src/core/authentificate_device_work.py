...
class AuthentificateDeviceOutcome:
    ...
    (shared object); ``apply_main_thread`` only emits on the core bus.
    """
...
def run(
    ...
    """
    Authentificate a device over ADB (call from a worker thread). Does not touch the core signal bus.
    """
    try:
...
def apply_main_thread(
    ...
    """
    Emit authentification outcome on the core bus (call from the Qt main thread only).
    """
